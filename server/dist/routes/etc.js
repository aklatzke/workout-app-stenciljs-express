...
exports.default = (r, db) => {
    r.get('/seed/data', async (req, res) => {
        let data = await _fsExtra2.default.readFile(__dirname + "/../../resources/workouts.csv");
        ...
        for (let key in workouts) {
            ...
            let timestamp = (0, _moment2.default)(key.split("|")[1], "YYYY-MM-DD h-mmm-ss").unix();
    ...
    r.get("/seed/exercises", async (req, res) => {
        let data = await _fsExtra2.default.readFile(__dirname + "/../../resources/exercises.csv");
        let rows = data.toString().split("\n");
